...
<html lang="zh-CN">
<head><meta charset="UTF-8"><title>旭儿导航</title>
<style>
*{margin:0;padding:0;box-sizing:border-box}body{font-family:system-ui;background:#f5f7fa;padding:20px}.container{max-width:1200px;margin:0 auto}.header{background:linear-gradient(135deg,#667eea,#764ba2);color:#fff;padding:40px;border-radius:20px;margin-bottom:30px;text-align:center}.header h1{font-size:36px}.section{background:#fff;border-radius:16px;padding:24px;margin-bottom:24px}.section h2{margin-bottom:20px;font-size:20px;border-left:4px solid #667eea;padding-left:12px}.grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:16px}.card{background:#f8f9fa;border-radius:12px;padding:16px}.card:hover{transform:translateY(-2px)}.card a{text-decoration:none;color:#333;display:block}.card h3{font-size:16px;color:#667eea}.post-item{border-bottom:1px solid #eee;padding:12px 0;display:flex;gap:16px;align-items:center}.post-cover{width:70px;height:50px;object-fit:cover;border-radius:8px;background:#e2e8f0}.post-info{flex:1}.post-title{font-weight:600;color:#333;text-decoration:none}.post-title:hover{color:#667eea}.post-meta{font-size:12px;color:#888;margin-top:6px}.admin-btn{display:inline-block;margin-top:20px;background:#667eea;color:#fff;padding:10px 24px;border-radius:30px;text-decoration:none}
</style></head>
<body><div class="container"><div class="header"><h1>📚 旭儿导航</h1><p>精选网站 · 优质博客</p></div>
<div class="section"><h2>📖 最新文章</h2>${publishedPosts.map(p => `<div class="post-item">${p.coverImage ? `<img src="${escapeHtml(p.coverImage)}" class="post-cover" onerror="this.style.display='none'">` : '<div class="post-cover" style="display:flex;align-items:center;justify-content:center">📄</div>'}<div class="post-info"><a href="/post/${p.id}" class="post-title">${escapeHtml(p.title)}</a><div class="post-meta">${p.category ? `🏷️ ${escapeHtml(p.category)} · ` : ''}📅 ${new Date(p.createdAt).toLocaleDateString()}</div></div></div>`).join('')}${publishedPosts.length === 0 ? '<div style="padding:20px;text-align:center">暂无文章</div>' : ''}</div>
...
function renderPosts(f){let list=f?allPosts.filter(p=>p.title.includes(f)):allPosts;document.getElementById('postList').innerHTML=list.map(p=>'<tr><td><strong>'+escape(p.title)+'</strong></td><td>'+escape(p.category||'未分类')+'</td><td><span class="status-badge '+(p.status==='published'?'status-published':'status-draft')+'">'+(p.status==='published'?'已发布':'草稿')+'</span></td><td>'+new Date(p.createdAt).toLocaleDateString()+'</td><td><button onclick="openEditModal('+p.id+')" style="background:#ed8936">编辑</button> <button class="delete-btn" onclick="deletePost('+p.id+')">删除</button></td></tr>').join('');}
...
async function saveEdit(){let id=document.getElementById('editId').value;let title=document.getElementById('editTitle').value.trim();let category=document.getElementById('editCategory').value.trim();let coverImage=document.getElementById('editCoverImage').value.trim();let status=document.getElementById('editStatus').value;let content=document.getElementById('editContent').value;if(!title||!content){alert('请填写标题和内容');return;}let r=await fetch('/api/blog/'+id,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({title,category,coverImage,status,content})});if(r.ok)location.reload();else alert('保存失败');}
async function addPost(){let title=document.getElementById('postTitle').value.trim();let category=document.getElementById('postCategory').value.trim();let coverImage=document.getElementById('postCoverImage').value.trim();let status=document.getElementById('postStatus').value;let content=document.getElementById('postContent').value;if(!title||!content){alert('请填写标题和内容');return;}let r=await fetch('/api/blog',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({title,category,coverImage,status,content})});if(r.ok)location.reload();else alert('发布失败');}
...
async function handleApi(request, kv) {
    ...
    if (request.method === 'PUT' && path.startsWith('/api/blog/')) {
        ...
        if (idx !== -1) {
            posts[idx] = { ...posts[idx], title: body.title, content: body.content, category: body.category, coverImage: body.coverImage, status: body.status, updatedAt: new Date().toISOString() };
            await kv.put('blog_posts', JSON.stringify(posts));
        }
        return new Response(JSON.stringify({ code: 200 }), { headers: { 'Content-Type': 'application/json' } });
    }
