...
<div style="overflow-x:auto"></td><thead><tr><th>ID</th><th>名称</th><th>网址</th><th>分类</th><th>排序</th><th>操作</th></tr></thead><tbody id="sitesList"></tbody></table></div></div>
...
function renderSites(){let sorted=[...allSites];sorted.sort((a,b)=>(a.sort_order||9999)-(b.sort_order||9999));let html='';for(let s of sorted){html+='<tr><td>'+s.id+'</td><td><strong>'+escape(s.name)+'</strong></td><td><a href="'+escape(s.url)+'" target="_blank">'+escape(s.url).substring(0,50)+'</a></td><td>'+escape(s.catelog)+'</span></span></span></span></td><td>'+(s.sort_order||9999)+'</span></span><td><td class="actions"><button class="btn-danger" onclick="deleteSite('+s.id+')">删除</button></td></tr>';}document.getElementById('sitesList').innerHTML=html;}
